...
public extension CGImage {
    ...
    static func create(from url: URL) throws -> CGImage {
        ...
        case "jpg":
            imageRaw = CGImage(jpegDataProviderSource: dataProvider,
                               decode: nil,
...
//    }
    ...
    static func create(pixelSource: inout DSPDoubleSplitComplex,
        ...
        let multiplier = [Double](repeating: Double((1 / denominator) * 255),
                                 count: pixelCount / 2)
        ...
        let floatPixels = [Double](fromSplitComplex: pixelSource,
                                  scale: 1,
                                  count: pixelCount)
...
extension CGImage {
    ...
    public func scaled(size: CGSize, interpolationQuality: CGInterpolationQuality = .high) throws -> CGImage {
...
public extension CGImage {
    ...
    func pixels<F>() throws -> [F] where F : vDSP_FloatingPointConvertable {
        ...
        let pixelsArray = Array(UnsafeBufferPointer(start: CFDataGetBytePtr(pixelData),
                                                    count: pixelCount))
        let pixels = vDSP.integerToFloatingPoint(pixelsArray,
...
public extension CGImage {
    ...
    func grayScale() throws -> CGImage {
        ...
        var sourceBuffer = try vImage_Buffer(cgImage: self,
                                             format: format)
        var scaledSourceBuffer = try vImage_Buffer(width: Int(sourceBuffer.width),
        ...
        var destinationBuffer = try vImage_Buffer(width: Int(scaledSourceBuffer.width),
        ...
        vImageMatrixMultiply_ARGB8888ToPlanar8(&scaledSourceBuffer,
                                               &destinationBuffer,
                                               &coefficientsMatrix,
                                               divisor,
                                               preBias,
...
private extension CGImage {
    ...
    static func create(
        ...
        if orientation == .right || orientation == .left {
            outputBuffer = try vImage_Buffer(width: Int(planarBuffer.height),
            ...
            outputBuffer = try vImage_Buffer(width: Int(planarBuffer.width),
